...
export const useChatBot = (isVisible, selectedLanguage = 'en', switchToScene) => {
    ...
    if (isMutedNow) {
      ...
      if (message.audioUrl) {
        ...
        if (selectedVoice) {
          ...
          console.log('NO LISA VOICE FOUND FOR PLAYBACK - NOT SPEAKING');
        }
        
        // Only proceed if we have Lisa voice
        if (selectedVoice) {
          const utterance = new SpeechSynthesisUtterance(message.content);
          utterance.lang = 'en-US';
          utterance.volume = 1.0; // Full volume for clarity
          ...
          utterance.voice = selectedVoice;
          console.log('Using Lisa voice for playback:', selectedVoice.name, selectedVoice.lang);
        ...
        setTimeout(() => {
          ...
          console.log('NO LISA VOICE FOUND FOR PLAYBACK - NOT SPEAKING TO AVOID OTHER VOICES');
        }
      }
    } else {
  ...
  useEffect(() => {
    if (isVisible && messages.length === 0) {
      ...
      // Directly trigger TTS for welcome message
      setTimeout(() => {
        if ('speechSynthesis' in window) {
          const voices = speechSynthesis.getVoices();
          console.log('Available voices for welcome message:', voices.map(v => `${v.name} (${v.lang}) - ${v.voiceURI}`));
          ...
          if (selectedVoice) {
            console.log('Found Lisa voice for welcome message:', selectedVoice.name);
          }
          ...
          if (!selectedVoice) {
            ...
            if (selectedVoice) {
              console.log('Found Serena voice for welcome message:', selectedVoice.name);
            }
          }
          ...
          if (!selectedVoice) {
            ...
            if (selectedVoice) {
              console.log('Found Karen voice for welcome message:', selectedVoice.name);
            }
          }
          
          // 4. If no specific voices, find ANY female voice (no male voices)
          if (!selectedVoice) {
            console.log('Looking for any female voice for welcome message...');
            selectedVoice = voices.find(voice => {
              const voiceName = voice.name.toLowerCase();
              const voiceURI = voice.voiceURI.toLowerCase();
            ...
            if (selectedVoice) {
              console.log('Found female voice for welcome message:', selectedVoice.name);
            }
          }
          
          // Only proceed if we have a female voice
          if (selectedVoice) {
            const utterance = new SpeechSynthesisUtterance(welcome.content);
            utterance.lang = 'en-US';
            utterance.volume = 1.0; // Full volume for clarity
            utterance.rate = 0.9; // Slightly slower for sweetness and clarity
            utterance.pitch = 1.1; // Slightly higher pitch for sweetness
            ...
            utterance.voice = selectedVoice;
            console.log('Using selected female voice for welcome message:', selectedVoice.name, selectedVoice.lang);
          
            utterance.onstart = () => {
              setIsAudioPlaying(true);
              setCurrentlyPlaying(welcome.id);
            };
            utterance.onend = () => {
              setIsAudioPlaying(false);
              setCurrentlyPlaying(null);
              // Automatically mute the welcome message after it finishes speaking
              setMutedMessages(prev => new Set([...prev, welcome.id]));
            };
            
            speechSynthesis.cancel();
            speechSynthesis.speak(utterance);
            console.log('Welcome message TTS initiated');
          } else {
            console.log('NO FEMALE VOICE FOUND FOR WELCOME MESSAGE - NOT SPEAKING TO AVOID MALE VOICE');
          }
        }
